...
import 'package:espe_book/models/ubicacion.dart';
import 'package:espe_book/models/usuario.dart';
import 'package:espe_book/resources/repository/auth_repository.dart';
import 'package:espe_book/resources/services/socket_service.dart';
...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  AuthBloc() : super(const AuthState(ubicaciones: [])) {
    ...
    on<MarcarNotificacionesPendienteFalse>(_onMarcarNotificacionesPendienteFalse);
  ...
  void _onAuthConnectEvent(AuthConectEvent event, Emitter<AuthState> emit) {
    socketService.connect();
  }

  void _onAuthDisconnectEvent(AuthDisconnectEvent event, Emitter<AuthState> emit) {
    socketService.disconnect();
  }

  void _onAuthAddUbicacionEvent(AuthAddUbicacionEvent event, Emitter<AuthState> emit) {
    if (state.ubicaciones.any((ubicacion) => ubicacion.uid == event.ubicacion.uid)) return;

    emit(state.copyWith(ubicaciones: [event.ubicacion, ...state.ubicaciones]));
  }

  void _onAuthDeleteUbicacionEvent(AuthDeleteUbicacionEvent event, Emitter<AuthState> emit) {
    emit(state.copyWith(
        ubicaciones: state.ubicaciones.where((ubicacion) => ubicacion.uid != event.uid).toList()));
  }
  ...
  void _onMarcarSalasPendienteFalse(MarcarSalasPendienteFalse event, Emitter<AuthState> emit) async {
  ...
  void _onAuthAddTelefonoEvent(AuthAddTelefonoEvent event, Emitter<AuthState> emit) {
    final usuario = state.usuario!.copyWith(
  ...
  void _onIsSalasPendiente(IsSalasPendiente event, Emitter<AuthState> emit) async {
    final usuario = state.usuario!.copyWith(
  ...
  void _onMarcarSalasPendienteTrue(MarcarSalasPendienteTrue event, Emitter<AuthState> emit) async {
    final usuario = state.usuario!.copyWith(isSalasPendiente: true);

    emit(state.copyWith(usuario: usuario));
  }

  void _aonAddTelefonoFamilyEvent(AuthAddTelefonFamilyEvent event, Emitter<AuthState> emit) {
    final telefono = event.telefono.trim();

    final telefonos = state.usuario!.telefonos.where((t) => t.isNotEmpty).toList();
